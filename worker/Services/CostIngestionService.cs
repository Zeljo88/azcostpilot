...
public sealed class CostIngestionService(
    HttpClient httpClient,
    IDbContextFactory<AppDbContext> dbContextFactory,
    ISecretEncryptionService secretEncryptionService,
    ILogger<CostIngestionService> logger) : ICostIngestionService
{
    private readonly HttpClient _httpClient = httpClient;
    private readonly IDbContextFactory<AppDbContext> _dbContextFactory = dbContextFactory;
    private readonly ISecretEncryptionService _secretEncryptionService = secretEncryptionService;
    private readonly ILogger<CostIngestionService> _logger = logger;

    public async Task<int> IngestLast7DaysAsync(CancellationToken cancellationToken)
    {
        var utcToday = DateTime.UtcNow.Date;
        var endDate = DateOnly.FromDateTime(utcToday);
        ...
        var targets = await db.Subscriptions.AsNoTracking()
            .Join(
                ...
                (subscription, connection) => new IngestionTarget(
                    subscription.UserId,
        ...
        foreach (var target in targets)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "Ingested {Count} daily cost rows for subscription {SubscriptionId}.",
                    costs.Count,
                    target.AzureSubscriptionId);
            }
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Cost ingestion failed for subscription {SubscriptionId}.",
                    target.AzureSubscriptionId);
            }
        }

        _logger.LogInformation(
            "Day 3 ingestion run completed. Processed {Processed} subscription(s) out of {TargetCount}.",
            processed,
            targets.Count);
        return processed;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new InvalidOperationException(
                $"Azure token request failed ({(int)response.StatusCode}) for tenant {tenantId}. {Truncate(errorBody)}");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            var errorBody = await response.Content.ReadAsStringAsync(cancellationToken);
            throw new InvalidOperationException(
                $"Cost query failed ({(int)response.StatusCode}) for subscription {subscriptionId}. {Truncate(errorBody)}");
        }
    ...
    private sealed record IngestionTarget(
        Guid UserId,
